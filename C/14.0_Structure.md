...
int main()
{
	...
	*/
	...
	printf("%zd\n", sizeof(union my_union)); // 8  <- 가장 큰 자료형으로 배정됨.
...
- 각각의 이름이 동일한 메모리 공간을 참조하므로, 접근 방식만 다를 뿐 실제 데이터는 동일  
  메모리 효율성이 매우 높아, 같은 데이터를 다양한 방식으로 다루는 데 유용
...
- enum(열거형, Enumeration)은 개발자가 특정한 상수들의 집합을 정의할 때 사용하는 자료형  
  즉, `의미 있는 이름을 숫자 값에 할당`할 수 있도록 해주는 기능  
  ex) yellow라는 이름으로 2를 사용하게 해준 것.
``` c
#include <stdio.h>
...
### 📌 14.22 함수 포인터의 사용 방법(Function Pointers)
...
int main()
{
	...
	void (*pf)(char*);  // 포인터로만 실행시키기 위해서, 파라미터의 사이즈와 반환값의 형태를 미리 적어줘야한다.

	pf = ToUpper;           // 함수 이름 자체가 주소를 가리킨다.
	// pf = &ToUpper;       // 주소를 직접 찍어준다.
	// pf = ToUpper(str);    <- 반환값을 넣겠다는 거니까 안된다.
	...
	(*pf)(str);  // 포인터니까 indirection 후 사용한다. (권장)
	//pf(str);   // 주소 자체도 함수명이니까 그냥 바로 쓴다.
	...
	/*
		...
		- 공통된 패턴(함수)들을 묶어서 하나의 함수로 다시 만들 수는 없을까.
		- 
	*/
...
int main()
{
	...
	*/
	...
	/*
	- typedef는 compiler가 처리를 해준다. define은 단순히 복사 붙혀넣기.
...
/*
    - 복잡한 선언은 typedef로 잘게 쪼개서 보면 편하다.
...
typedef char c3[3];  // char의 3개짜리 배열을 c3 자료형으로 선언.
...
int main()
{
	...
	*/
	...
	// 배열
	int* ap[10];	 // 10개짜리 배열, int포인터가 들어있는.    Identifier ap is an array of pointers
	...
	// 함수
	float* fp(float);  // 함수 fp, float이 들어가고, float포인터가 return.    fp is a function that returns a pointer

	typedef float* pfloat;
	pfloat fp2(float);


	// 정수형 인자를 받아 실행되는 함수의 주소를 저장하는 포인터
	void (*pf)(int);  // 함수에 대한 포인터인데, int가 들어가고, return이 없는.
	/*
		void (*pf)(int);
			  1           1. pointer to
	...
	*/
	...
	// 뒤에 괄호면 인식자는 함수다.
	int* (*x[10])(void);  // 10개짜리 배열 / 함수인데, 매개변수 없이, return이 int 포인터인.
	/*
		int* (*x[10])(void);
				 1            1. array of
			  2               2. pointers to
	...
	*/
	...
	int board[6][4];// 6×4 배열 (2차원 배열)
	int** ptr;		// int를 가리키는 포인터의 포인터

	int* risks[10];	// 정수를 가리키는 포인터를 저장하는 배열
	int(*rusk)[10];	// int[10]을 가리키는 포인터

	int* off[3][4]; // 3x4 배열, 요소는 int* (포인터 배열)
	int(*uff)[3][4];// 3x4 배열을 가리키는 포인터
	int(*uof[3])[4];// 크기 3의 배열, 요소는 int[4]을 가리키는 포인터
...
### 📌 14.25 qsort 함수 포인터 연습문제 (quick sort)
``` c
#define _CRT_SECURE_NO_WARNINGS
#include <stdio.h>
#include <stdlib.h>
